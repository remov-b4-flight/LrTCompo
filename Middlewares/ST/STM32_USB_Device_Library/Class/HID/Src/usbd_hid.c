...
/* USER CODE END 0 LrTMAX */
{
  ...
  pdev->pHIDClassData = USBD_malloc(sizeof(USBD_HID_HandleTypeDef));
  ...
  ((USBD_HID_HandleTypeDef *)pdev->pHIDClassData)->state = HID_IDLE;
...
/* USER CODE END 0 LrTMAX */
{
  USBD_HID_HandleTypeDef *hhid = (USBD_HID_HandleTypeDef *) pdev->pHIDClassData;
  uint16_t len = 0U;
  uint8_t *pbuf = NULL;
